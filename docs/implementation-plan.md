...
- `duration_triggered` 作为独立小步补齐后，复杂触发的剩余工作转向持久化和横向扩展
- 不做权限、家庭多租户、用户画像
- 不做长期记忆、语义事实
...
| 5.2 Trigger | `scenes/runtime/sequential.py` + `scenes/runtime/edge.py` + `scenes/runtime/windowed_all.py` + `scenes/runtime/duration.py` |
